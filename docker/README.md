...
reach it — never API Gateway, never the public internet. On the EC2 dev/qa
boxes this "just works" the same way node's target group does today: the
container's host port (4001 dev / 4002 qa) is what the LB's target group
points at.
node-app reads that LB's DNS name via `JAVA_SERVICE_URL` (see each
node-app `.env.example`) — never a specific java-app instance — so the same
app code works whether 1 instance answers behind it (dev/qa today) or 2+
(production).
