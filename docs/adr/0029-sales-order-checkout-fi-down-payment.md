...
(Product master data itself — `product`/`product_sales_org`/`product_uom`, the fields *this* pricing addition builds on top of — is extracted separately, folded into the existing Nightly Bulk SAP Extraction batch, ADR-0017. See that ADR's own update note and `0002-scheduled-jobs.md`'s Product Extraction detail.)
