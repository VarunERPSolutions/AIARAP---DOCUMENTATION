...
## Resolved: the ripple into Vendor-side access/security

[ADR-0014](0014-tenant-managed-payer-vendor-user-onboarding-offboarding.md), [0015](0015-payer-vendor-admin-bootstrap-via-access-request.md), and [0016](0016-payer-vendor-admin-user-termination.md) are all written generically as "Payer/Vendor" — onboarding, admin bootstrap, and termination mechanics that apply equally to both. **Vendor-side access moves to Phase 2 alongside the rest of AP.** There's no Phase 1 feature for a Vendor User to reach once onboarded — building that flow now would mean a Vendor Admin logs into a portal with nothing in it. All three ADRs stay as generically-written designs (nothing about their *mechanics* changes — the same Access Request/Admin-bootstrap/termination shape will apply to Vendor once AP ships), they're just **Payer-only for Phase 1 in practice**: `Vendor` drops out of Milestone 0's Phase 1 entity list, and Milestone 1's Access Request/Admin flows are scoped to Payer only until Phase 2.
