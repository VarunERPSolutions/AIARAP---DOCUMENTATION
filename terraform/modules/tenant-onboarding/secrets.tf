# One secret per connection, handed to the Tenant once over a secure channel.
...
resource "aws_secretsmanager_secret" "conn" {
  ...
  name     = "varunerp/tenants/${var.subdomain}/${each.value.conn_key}/${each.value.env}"
}
...
resource "aws_secretsmanager_secret_version" "conn" {
  ...
  secret_string = jsonencode({
    ...
    token_url     = "https://${var.cognito_domains[each.value.env]}/oauth2/token"
    ...
    api_host      = each.value.is_prod ? "${var.subdomain}.${var.base_domain}" : "${each.value.env}.${var.subdomain}.${var.base_domain}"
