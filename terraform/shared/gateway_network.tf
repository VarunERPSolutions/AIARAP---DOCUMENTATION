...
# node-app/react-app without those instances becoming directly reachable —
# satisfies ADR-0005's "backends unreachable except through the gateway"
# requirement.
#
# java-app is deliberately NOT part of this path — per ADR-0039, Java has no
# Tenant-facing inbound routing at all; its only inbound caller is NestJS,
# internally (ADR-0039 section 4), not the public API Gateway.
#
