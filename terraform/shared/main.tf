...
# integration forwards. This is what keeps the API/authorizer count at 3
# instead of ballooning to one-per-environment.
locals {
  backends = toset(["node", "java", "sap"])
}
...
locals {
  backend_envs = merge(
    ...
    { for env, cfg in var.java_environments : "java-${env}" => merge(cfg, { backend = "java", env = env }) },
    {
  ...
  node_java_environments = {
    node = var.node_environments
    java = var.java_environments
  }
}
...
check "prd_not_sharing_devqa_instance" {
  assert {
    condition = alltrue([
      for backend in ["node", "java"] :
      !contains(keys(local.node_java_environments[backend]), "prd") || (
        local.node_java_environments[backend]["prd"].instance_id != try(local.node_java_environments[backend]["dev"].instance_id, null)
      )
    ])
    error_message = "prd's instance_id matches dev's — prod must be its own dedicated instance, not the dev/qa shared box."
  }
}
